const Footer = () => (
  <footer className="navy-gradient py-12 px-4">
    <div className="container mx-auto">
      <div className="grid md:grid-cols-3 gap-8 mb-8">
        <div>
          <h3 className="font-serif text-xl font-bold text-primary-foreground mb-2">
            SHREEDHAR <span className="gold-text">Group</span>
          </h3>
        ...
        <div>
          <h4 className="font-sans text-sm uppercase tracking-wider text-secondary mb-3">Quick Links</h4>
          <div className="space-y-2">
            {["About", "Projects", "Why Bhankrota", "Calculator", "Contact"].map((l) => (
              <a
                key={l}
                href={`#${l.toLowerCase().replace(/ /g, "-")}`}
                className="block font-sans text-sm text-primary-foreground/50 hover:text-secondary transition-colors"
              >
                {l}
              </a>
            ))}
        ...
        <div>
          <h4 className="font-sans text-sm uppercase tracking-wider text-secondary mb-3">SEO Keywords</h4>
          <p className="font-sans text-xs text-primary-foreground/30 leading-relaxed">
